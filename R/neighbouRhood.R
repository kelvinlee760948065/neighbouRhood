...
#'             direction: is p_lt greater than p_gt?
...
cal_zscore <- function(dat_baseline,dat_perm){
  ...
  dat_perm_1=dat_perm[, ':='(m=mean(ct),s=sd(ct)), by = .(group, FirstLabel, SecondLabel)] 
  dat_perm_1=dat_perm[, `:=`(ct=NULL,run=NULL)] %>%
  	unique()
  
  dat_combine=dat_baseline[dat_perm_1,on=c('group', 'FirstLabel', 'SecondLabel')]
  dat_combine[,z:=(ct-m)/s]
	
  
}

#' Calculates cell propotion of cell type Y among neighbours of cell type X (written by KelvinLee)
#' Afterwards, calculates mean of each X-Y interaction within each sample
neighbour_proportion <- function(dat_baseline){
  ...
  table_rate=table_tmp[,4:ncol(table_tmp)] %>%
    apply(1,function(x){
        x/sum(x)
    }) %>% t() %>% data.table()
